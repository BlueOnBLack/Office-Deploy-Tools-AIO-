...
:UsingPS
>"%windir%\Temp\script.ps1" echo Get-WmiObject -Query "select * from %QueryClass% where %QueryString%" ^| format-table -Property %QueryTable%
powershell -noprofile -executionpolicy bypass -file "%windir%\Temp\script.ps1"
goto :eof

:UsingVBS
 >"%windir%\Temp\script.vbs" echo on error resume next
>>"%windir%\Temp\script.vbs" echo strQuery = "Select " ^+ "%QueryTable%" + " from " ^+ "%QueryClass%" ^+ " where " ^+ "%QueryString%"
>>"%windir%\Temp\script.vbs" echo Set objArray= GetObject("winmgmts:\\.\root\CIMV2").ExecQuery(strQuery,,48)
>>"%windir%\Temp\script.vbs" echo For each obj in objArray
>>"%windir%\Temp\script.vbs" echo 	result = ","
>>"%windir%\Temp\script.vbs" echo 	For each Prop in obj.Properties_
>>"%windir%\Temp\script.vbs" echo 		if NOT result = "," Then
>>"%windir%\Temp\script.vbs" echo 			result = result ^& Prop.Value ^& ","
>>"%windir%\Temp\script.vbs" echo 		End if
>>"%windir%\Temp\script.vbs" echo 		if result = "," Then
>>"%windir%\Temp\script.vbs" echo 			result = Prop.Value ^& ","
>>"%windir%\Temp\script.vbs" echo 		End if
>>"%windir%\Temp\script.vbs" echo 	Next
>>"%windir%\Temp\script.vbs" echo 	if NOT result = "," Then
>>"%windir%\Temp\script.vbs" echo 		WScript.Echo "%QueryTable%"
>>"%windir%\Temp\script.vbs" echo 		WScript.Echo result
>>"%windir%\Temp\script.vbs" echo end if
>>"%windir%\Temp\script.vbs" echo Next
cscript //Nologo "%windir%\Temp\script.vbs"
goto :eof