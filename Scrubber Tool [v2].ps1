...
if ($HashList) {
  foreach ($hash in $HashList.GetEnumerator()) {
    if (($hash.Value -notlike "*OfficeClickToRun.exe*") -and (
      $false -eq ($hash.Value|CheckDelete) )) {
      continue }
    $value = $hash.Value;
    $data  = $value.Split( )
    if ($data) {
    ...
    try {
      ...
      $arrayList.Add($hash.Key,$prefix[2]) }
    catch {}
}}}

foreach ($hash in $arrayList.GetEnumerator()) {
  if ([IO.FILE]::Exists($OfficeClickToRun)) {
    $Proc = start $OfficeClickToRun -Arg $hash.Value -Wait -WindowStyle Hidden -PassThru -ea 0
...
SCHTASKS /Delete /TN FF_INTEGRATEDstreamSchedule /F *>$null
SCHTASKS /Delete /TN C2RAppVLoggingStart /F *>$null
SCHTASKS /Delete /TN "Office 15 Subscription Heartbeat" /f *>$null
SCHTASKS /Delete /TN "Microsoft Office 15 Sync Maintenance for {d068b555-9700-40b8-992c-f866287b06c1}" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\OfficeInventoryAgentFallBack" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\OfficeTelemetryAgentFallBack" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\OfficeInventoryAgentLogOn" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\OfficeTelemetryAgentLogOn" /f *>$null
SCHTASKS /Delete /TN "Office Background Streaming" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\Office Automatic Updates" /f *>$null
SCHTASKS /Delete /TN "\Microsoft\Office\Office ClickToRun Service Monitor" /f *>$null
SCHTASKS /Delete /TN "Office Subscription Maintenance" /f *>$null
}
