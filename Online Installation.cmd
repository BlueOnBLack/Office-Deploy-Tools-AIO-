...
if not defined xTmp (
	if not exist BuildInfo.ini (
		echo.
		echo Supported C2R Versions 	 :: [2013, 2016, 2019, 2021,2024] [Fallback - 2021]
		echo Supported System Bit 	 :: [32, 64, Auto] [Fallback - System Arch]
		echo Supported Language List  :: [Open "Read Me" File] [Fallback - System Language]
		echo.
		...
		echo "%~n0" 2013 Auto hebrew   - Office $$$ 2013 - X86-X64 - Hebrew
		echo.
		echo "%~n0" 1 		 - Lite - Office 2021 - [System Arch] - [System Language]
		echo "%~n0" 2 2016 64 	 - Full - Office 2016 - X64 - [System Language]
		echo "%~n0" 3 2013 Auto he-il  - VIP  - Office 2013 - X86-X64 - Hebrew
		echo "%~n0" 4 Russian  	 - 365  - Russian
		echo "%~n0" 6 2024 32 en-gb    - ALL  - Office 2024 - X86 - English UK
		echo.
...
for /f "tokens=* skip=3" %%g in ('type "%windir%\Temp\result"') do set "BuildNumber=%%g"
...
if not defined userSelected (
	...
	echo :: E - Exit
	echo:
	CHOICE /C 12345E /M "Select build Year To Continue, Press E to Exit ::"
	...
	if !errorlevel! EQU 5 call :updateYear 2024
	if !errorlevel! EQU 6 (pause & exit /b)
	call :UpdatePID
)
...
1>nul 2>&1 del /q /s "%windir%\Temp\v!xBit!.cab"
1>nul 2>&1 del /q /s "%windir%\Temp\VersionDescriptor.xml"

:: Nethood 1XX
if /i '!productyear!' NEQ '2013' (
	1>nul 2>&1 tools\wget --quiet --tries=3 --retry-connrefused --user-agent=%user_agent% --output-document="%windir%\Temp\VersionDescriptor.xml" %VersionReleaseApi%
)

:: Nethood 2XX
if /i '!productyear!' EQU '2013' (
	1>nul 2>&1 tools\wget --quiet --user-agent=%user_agent% --output-document="%windir%\Temp\v!xBit!.cab" "officecdn.microsoft.com.edgesuite.net/pr/!ProductId!/Office/Data/v!xBit!.cab"
	1>nul 2>&1 expand -f:VersionDescriptor* "%windir%\Temp\v!xBit!.cab" "%windir%\Temp"
)
...
:: Nethood 1XX
if /i '!productyear!' NEQ '2013' (
	for /f "tokens=2 delims=:, " %%g in ('type "%windir%\Temp\VersionDescriptor.xml" ^| find /i "AvailableBuild"') do (
...
:: Nethood 2XX
if /i '!productyear!' EQU '2013' (
	for /f  "tokens=2 delims= " %%g in ('type "%windir%\Temp\VersionDescriptor.xml" ^| find /i "Available Build"') do (
		if not defined version (
			set "version=%%g"
...
if defined setupmode (
	...
	echo !setupMode!|>nul find /i "1" && echo Install Word,Excel,Powerpoint !ProductYear! VL %langCd% x!xBit! v!version!
	
	echo !setupMode!|>nul find /i "2" && (
		if /i '!ProductYear!' EQU '2013' (
			echo Install Office professional plus !ProductYear! VL %langCd% x!xBit! v!version!
		) else if /i '!ProductYear!' EQU '2016' (
			echo Install Office professional plus !ProductYear! VL %langCd% x!xBit! v!version!
		) else (
			echo Install Office professional plus !ProductYear! Perpetual VL %langCd% x!xBit! v!version!
		)
	)
	
	echo !setupMode!|>nul find /i "3" && echo Install ProjectPro,visioPro !ProductYear! VL %langCd% x!xBit! v!version!
	echo !setupMode!|>nul find /i "4" && echo Install Microsoft 365 Business %langCd% x!xBit! v!version!
	echo !setupMode!|>nul find /i "5" && echo Install !selected_Products! !ProductYear! VL %langCd% x!xBit! v!version!
	echo !setupMode!|>nul find /i "6" && echo Install ProPlus,ProjectPro,visioPro !ProductYear! VL %langCd% x!xBit! v!version!
)
...
:cleanVariable
set vars=xVal, xBit, sBit, sLng, lLngID, lBit, lLng, lYear, equal, Auto, stream, langId, langCd, version, cscript, platform, file_list, iso_file, ProductId, SysLanCD, SysLanIdHex, ProductYear, SupportedBit, VersionReleaseApi, SupportedbuildYear, key, ProductReleaseIds, ProductReleaseIds_
for %%v in (%vars%) do (set %%v=)
goto :eof

:stpDownload
set stpp="%windir%\Temp\setup.exe"
...
:UpdateBuildInfo
set "SupportedBit=32, 64, Auto"
set "SupportedbuildYear=2013, 2016, 2019, 2021, 2024"
		...
		if defined setupmode (
			echo !setupMode!|>nul find /i "1" && (
				for %%g in (word, excel, powerpoint) do (
					...
					>"%windir%\Temp\tmp" call :volume_license_serial_list
					for /f "tokens=1,2,3 delims=*" %%x in ('type "%windir%\Temp\tmp"') do (
						if /i '%%x' EQU '%%g' (
...
if not defined equal (
	for %%g in (2013, 2016, 2019, 2021, 2024) do (
...
call :compareXY "%pharse:~5,4%" "Auto"
if defined equal (
	call :updateBit Auto
	goto :InputPharser_
)
...
call :compareXY "%pharse:~7,4%" "Auto"
if defined equal (
	call :updateBit Auto
...
:InputPharser__S
if defined Auto (
	call :compareXY "%pharse:~12%" ""
	if not defined equal (
		call :LangPharser %pharse:~12%
	)
)
...
for /f "usebackq tokens=*" %%# in (`"2>nul powershell -nop -c !COMMAND!"`) do set "SysLanIdHex=%%#"
...
>"%windir%\Temp\tmp" call :Kms_Servers_List
for /f "tokens=*" %%g in ('type "%windir%\Temp\tmp"') do (
...
if /i '!ProductYear!' EQU '2024' (
	set "ProductId=ea4a4090-de26-49d7-93c1-91bff9e53fc3"
)
...
>"%windir%\Temp\tmp" call :Language_List
for /f "tokens=1,2,3 delims=*" %%g in ('type "%windir%\Temp\tmp"') do if exist "office\data\!version!\i64%%h.cab" set lLngID=%%h
...
>"%windir%\Temp\script.ps1" echo Get-WmiObject -Query "!args_1:~1,-1!" ^| format-table -Property !args_2:~1,-1!
>"%windir%\Temp\result" 2>&1 powershell -noprofile -executionpolicy bypass -file "%windir%\Temp\script.ps1"
goto :eof
...
set args_2=%2
>nul 2>&1 del /q "%windir%\Temp\result"
>"%windir%\Temp\script.ps1" echo Get-WmiObject -Query "!args_1:~1,-1!" ^| ForEach-Object { ($_).!args_2:~1,-1!()}
>"%windir%\Temp\result" powershell -noprofile -executionpolicy bypass -file "%windir%\Temp\script.ps1"
goto :eof
...
set args_3=%3
>nul 2>&1 del /q "%windir%\Temp\result"
>"%windir%\Temp\script.ps1" echo Get-WmiObject -Query "!args_1:~1,-1!" ^| ForEach-Object { ($_).!args_2:~1,-1!("!args_3:~1,-1!")}
>"%windir%\Temp\result" powershell -noprofile -executionpolicy bypass -file "%windir%\Temp\script.ps1"
goto :eof
