...
if defined Windows_7_Or_Earlier (
	if /i '!ProductYear!' NEQ '2016' (
		if /i '!ProductYear!' NEQ '2013' (
			echo.
			echo Can't install office 2019 - 2024 on windows 7
			goto :end
		)
	)
...
REM for %%g in (2013, 2016, 2019, 2021, 2024) do (
...
(>nul expand !CabFile! -F:VersionDescriptor.xml "%windir%\Temp")  || goto :eof
if not exist "%windir%\Temp\VersionDescriptor.xml" goto :eof
...
if not defined DeliveryMechanism (
	type "%windir%\Temp\VersionDescriptor.xml"| >nul find /i "15." && (
		set "lYear=2013"
...
for /f "tokens=1,2 delims=*" %%g in ('type "%windir%\Temp\tmp"') do (
  (echo !DeliveryMechanism! | >nul find /i "%%h") && (
	set "ProductId=%%h" & set "lYear=2016" & set "ProductYear=2016"
    ...
    echo "%%g" | >nul find /i "DogfoodDevMain"  && (echo *** DogfoodDevMain Channel Found & set "lYear=" & goto :eof)
