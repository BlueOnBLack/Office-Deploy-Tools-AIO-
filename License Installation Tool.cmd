...
set HideMode=%4
::::::::::::::::::::::::::::::::::
::force use WMI PS ENGINE
rem set FORCE_PS=true
...
::::::::::::::::::::::::::::::::::
set fileLoc="%~dpfx0"
set tmpFile="%windir%\Temp\tmp"
set "SupportedbuildYear=2019,2021,2024"
::::::::::::::::::::::::::::::::::
set Invalid="Invalid"
...
if not exist "Tools\KmsHelper.vbs" (
	echo. & echo Missing Critical Files [KmsHelper.vbs]
	echo. & pause
	exit /b
)

if not exist "Tools\KmsHelper.ps1" (
	echo. & echo Missing Critical Files [KmsHelper.ps1]
	echo. & pause
	exit /b
)
...
set "WMI_PS=true" )
cscript .vbs | %PrintNul% find /i "There is no script engine" && (
set "WMI_PS=true" )

:: check if exist such Capability and install it [if necessary]
(DISM /Online /Get-CapabilityInfo /CapabilityName:VBSCRIPT~~~~ | %PrintNul% find /i "State : Not Present") && (
  echo:
  set "WMI_PS=true"
  rem echo *** Install VBSCRIPT~~~~ Capability
  %PrintNul% DISM /Online /Add-Capability /CapabilityName:VBSCRIPT~~~~
  echo:
)
if defined FORCE_PS (
 if /i '%FORCE_PS%' EQU 'TRUE' (
  set WMI_PS=true
 )
)
...
if not defined HideMode echo Log file saved at :: "%windir%\Temp\log"
if not defined HideMode echo please wait 6 seconds..
if not defined HideMode timeout /t 6 /NOBREAK
goto :eof

:cleanVariable
set vars=key, c2r_14_Key, c2r_15_Key, root_14, root_15, root_16, guid_14, guid_15, guid_16, cscript, ProductYear, KmsServer, V2010, UnInstallKeys, U_2010, U_2013, U_2016, U_2019, U_2021, U_2024
	...
	if defined SelectedX (
		echo !ProductReleaseIds! | find /i "%%x">nul && (
			...
			for %%m in (!ProductReleaseIds!) do (
				echo %%m | >nul find /i "%%x" && (
					set ProductYear=
					echo %%m | >nul find /i "2024" && set ProductYear=2024
...
if /i '%tArgs%' EQU '-Convert' (
	...
	set foundYear=
	for %%g in (2010, 2013, 2016, 2019, 2021, 2024) do (if /i '!tYear!' EQU '%%g' (set foundYear=true))
...
if defined c2r_16_Key (
	...
	if not defined HideMode echo.
	if not defined HideMode echo ##### Search For Office C2R [2016, 2019, 2021, 2024] Products
	...
	if defined xArgs (
		set ProductMatch=
		set ProductList=2016, 2019, 2021, 2024
...
echo '%nYear%' | find /i "2024">nul && set nProduct=!nProduct!!nYear!Volume.16
...
:inslic
set pStart=%time: =0%
>nul 2>nul del /q %windir%\temp\output
Rem Main Code Here
if not defined HideMode echo install license file for %1
if defined WMI_PS (
  >%windir%\temp\output powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/InstallLicense" "Licence\%1"
) else (
  >%windir%\temp\output %cscript% Tools\x!xBit!\ospp!V2010!.vbs /inslic:"Licence\%1"
)
Rem Main Code Here
...
%PrintLog% echo Start :: !pStart! --- End :: !pEnd! --- Total !_tdiff!
if defined WMI_PS (
  %printCmd_v2% powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/InstallLicense" "Licence\%1"
) else (
  %printCmd_v2% %cscript% Tools\x!xBit!\ospp!V2010!.vbs /inslic:"Licence\%1"
)

if exist %windir%\temp\output (
 %PrintLog% type %windir%\temp\output
 >nul 2>nul del /q %windir%\temp\output
)
%PrintLog% echo ******************************************
goto :eof
...
if defined WMI_PS (
  >%windir%\temp\output powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/InstallProductKey" !xKey!
) else (
  >%windir%\temp\output %cscript% Tools\x!xBit!\ospp!V2010!.vbs /inpkey:%xKey%
)
Rem Main Code Here
...
%PrintLog% echo Start :: !pStart! --- End :: !pEnd! --- Total !_tdiff!
if defined WMI_PS (
  %printCmd_v2% powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/InstallProductKey" !xKey!
) else (
  %printCmd_v2% %cscript% Tools\x!xBit!\ospp!V2010!.vbs /inpkey:%xKey%
)
...
Rem Main Code Here
set xKey=%1
if not defined HideMode echo Uninstall Key :: %xKey:~-5%
if defined WMI_PS (
  >%windir%\temp\output powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/UninstallProductKey" "!LicensingProductClass!" "partialProductKey like '%%%%!xKey!%%%%'"
) else (
  >%windir%\temp\output %cscript% Tools\x!xBit!\ospp!V2010!.vbs /unpkey:%xKey%
)
Rem Main Code Here
...
%PrintLog% echo Start :: !pStart! --- End :: !pEnd! --- Total !_tdiff!
if defined WMI_PS (
  %printCmd_v2% powershell -nop -ep bypass -file "Tools\KmsHelper.ps1" "/UninstallProductKey" "!LicensingProductClass!" "partialProductKey like '%%%%!xKey!%%%%'"
) else (
  %printCmd_v2% %cscript% Tools\x!xBit!\ospp!V2010!.vbs /unpkey:%xKey%
)
...
echo %tArgs% | >nul find /i "2024" && (
	if defined U_2024 goto :eof
	set U_2024=Done
)
...
if defined xYear (
	...
	>>%tmpFile% type "%windir%\Temp\result"
)
...
for /f "tokens=1,2 delims=," %%x in ('type %tmpFile%') do (
	:: search for un needed items to avoid echo. that break my for loop
	if /i '%%y' NEQ '' (
	    if     defined WMI_PS set "SelectedKey=%%x"
		if not defined WMI_PS set "SelectedKey=%%y"
	
		if /i "!xYear!" EQU "2014" (
			call :unpkey !SelectedKey!
...
set Product_Not_Found=
(type "%windir%\Temp\result"|find /i ",">nul) 					|| set Product_Not_Found=true
(type "%windir%\Temp\result"|find /i %invalid%>nul) 			&& set Product_Not_Found=true
(type "%windir%\Temp\result"|find /i %ProductError%>nul)		&& set Product_Not_Found=true
(type "%windir%\Temp\result"|find /i %ProductNotExist%>nul) 	&& set Product_Not_Found=true
(type "%windir%\Temp\result"|find /i %ProductNotFound%>nul) 	&& set Product_Not_Found=true
...
for /f "tokens=1,2,3,4,5,6,7,8 delims=," %%g in ('type "%windir%\Temp\result"') do (
	...
	if not defined WMI_PS (
	  ...
	  call :Activate_WMI_ %1 %2 !idVal! !CVal!
	  echo:
	)
	...
	if defined WMI_PS (
	  ...
	  set idVal=%%g
	  set "editionName=%%l"
	  set /a "Period=%%m/60/24"
	  
	  echo !editionName:~1, -8!
	  echo ....................................
	  echo Partial Product Key	= %%i
	  echo License / Genuine 	= %%h / %%j
	  echo Period Remaining 	= !Period! Days
	  echo Id 			= %%g
	  echo:
	  echo ^+^+^+ Activating ^+^+^+ && echo ...................
...
echo Excel*2024*F4DYN-89BP2-WQTWJ-GR8YC-CKGJG
...
echo PowerPoint*2024*CW94N-K6GJH-9CTXY-MG2VC-FYCWP
...
echo ProPlus*2024*QND99-8Q3Y9-M4MPQ-GQ7PH-R9GJY
...
echo ProjectPro*2024*6QNPD-VV372-HC9H8-RR893-RJWCF
...
echo ProjectStd*2024*PD3TT-NTHQQ-VC7CY-MFXK3-G87F8
...
echo SkypeforBusiness*2024*4NKHF-9HBQF-Q3B6C-7YV34-F64P3
...
echo Standard*2024*V28N4-JG22K-W66P8-VTMGK-H6HGR
...
echo VisioPro*2024*NPR26-FF7TB-6QBBV-W2FTG-WXGKG
...
echo VisioStd*2024*JMMVY-XFNQC-KK4HK-9H7R3-WQQTV
...
echo access*2024*82FTR-NCHR7-W3944-MGRHM-JMCWD
...
echo outlook*2024*D2F8D-N3Q3B-J28PV-X27HD-RJWB9
echo word*2019*PBX3G-NWMT6-Q7XBW-PYJGG-WXD33
echo word*2021*TN8H9-M34D3-Y64V9-TR72V-X79KV
echo word*2024*MQ84N-7VYDM-FXV7C-6K7CC-VFW9J
goto :eof
...
if /i '%3' EQU '' (
  if defined WMI_PS (
	>"%windir%\Temp\result" powershell -nop -ep bypass -file "tools\KmsHelper.ps1" "/QUERY_BASIC" %1 %2
  ) else (
	>"%windir%\Temp\result" cscript //nologo "tools\KmsHelper.vbs" "/QUERY_BASIC" %1 %2
  )
) else (
  if defined WMI_PS (
	>"%windir%\Temp\result" powershell -nop -ep bypass -file "tools\KmsHelper.ps1" "/QUERY_ADVENCED" %1 %2 %3
  ) else (
	>"%windir%\Temp\result" cscript //Nologo "tools\KmsHelper.vbs" "/QUERY_ADVENCED" %1 %2 %3
  )
)
Rem Main Code Here
...
if /i '%3' EQU '' (
  if defined WMI_PS (
	%printCmd_v2% powershell -nop -ep bypass -file "tools\KmsHelper.ps1" "/QUERY_BASIC" %1 %2
  ) else (
	%printCmd_v2% cscript //Nologo "tools\KmsHelper.vbs" "/QUERY_BASIC" %1 %2
  )
) else (
  if defined WMI_PS (
	%printCmd_v2% powershell -nop -ep bypass -file "tools\KmsHelper.ps1" "/QUERY_ADVENCED" %1 %2 %3
  ) else (
	%printCmd_v2% cscript //Nologo "tools\KmsHelper.vbs" "/QUERY_ADVENCED" %1 %2 %3
  )
)
...
set "lastErr="
if defined WMI_PS (
  set "activationCMD=2>nul powershell -nop -ep bypass -file "Tools\KmsHelper.PS1" "/ACTIVATE" "%1" "%2""
) else (
  set "activationCMD=2>nul cscript //Nologo "Tools\KmsHelper.vbs" "/ACTIVATE" "%1" "%2""
)
...
%PrintLog% echo Start :: !pStart! --- End :: !pEnd! --- Total !_tdiff!
if defined WMI_PS (
  %printCmd_v2% powershell -nop -ep bypass -file "Tools\KmsHelper.PS1" "/ACTIVATE" "%1" "%2"
) else (
  %printCmd_v2% cscript //Nologo "Tools\KmsHelper.vbs" "/ACTIVATE" "%1" "%2"
)
